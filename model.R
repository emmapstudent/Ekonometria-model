...
#Macierz korelacji zmiennych liniowych
install.packages("corrplot")
library(corrplot)
dane_cor <- subset(dane2, select = 
                     c(zasilek_rodzinny_wspolczynnik:plodnosc,
                       wspolczynnik_wyksztalcenia:swiadczenia_wychowawcze))
M<-cor(dane_cor)
...
reg5 <- lm(aktywnosc_zawodowa_kobiet ~ zasilek_rodzinny_wspolczynnik + 
             ...
             swiadczenia_wychowawcze, data = dane2)
summary(reg5)
#Testy formy funkcyjnej
#RESET
lmtest::resettest(reg1)
(rf <- resettest(reg5, power = 2:3, type = c("fitted")))
#(rr <- resettest(reg, power = 2:3, type = c("regressor")))
